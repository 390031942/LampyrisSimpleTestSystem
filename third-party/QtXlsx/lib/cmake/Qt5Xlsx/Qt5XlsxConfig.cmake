...
function(_qt5_Xlsx_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    set(_qt5_install_libs "${_qt5Xlsx_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5Xlsx_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5Xlsx_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5Xlsx_${Configuration}_${_lib}_PATH)
                    if(_Qt5Xlsx_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5Xlsx_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_Xlsx_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set(_static_deps
        ${_Qt5Xlsx_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    set(_static_link_flags "${_Qt5Xlsx_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::Xlsx APPEND PROPERTY INTERFACE_LINK_OPTIONS
...
if (NOT TARGET Qt5::Xlsx)
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        _qt5_Xlsx_process_prl_file(
            "${_qt5Xlsx_install_prefix}/lib/Qt5Xlsxd.prl" DEBUG
            _Qt5Xlsx_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5Xlsx_STATIC_DEBUG_LINK_FLAGS
        )

        _qt5_Xlsx_process_prl_file(
            "${_qt5Xlsx_install_prefix}/lib/Qt5Xlsx.prl" RELEASE
            _Qt5Xlsx_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5Xlsx_STATIC_RELEASE_LINK_FLAGS
        )
    endif()

    add_library(Qt5::Xlsx STATIC IMPORTED)
    set_property(TARGET Qt5::Xlsx PROPERTY IMPORTED_LINK_INTERFACE_LANGUAGES CXX)
    ...
    _populate_Xlsx_target_properties(RELEASE "Qt5Xlsx.lib" "" TRUE)



    _populate_Xlsx_target_properties(DEBUG "Qt5Xlsxd.lib" "" TRUE)
